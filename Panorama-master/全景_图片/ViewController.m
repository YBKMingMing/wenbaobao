...
//    }
    ...
    // 将self.vertexBufferID绑定到 GL_ARRAY_BUFFER
    glBindBuffer(GL_ARRAY_BUFFER, self.vertexBufferID);
    glBufferData(GL_ARRAY_BUFFER, numVertices*3*sizeof(GLfloat), vVertices, GL_STATIC_DRAW);
    /// 激活顶点位置属性
    glEnableVertexAttribArray(GLKVertexAttribPosition);
    //有四个参数(size,Data Type- GL_FLOAT,stride,pointer to the data)
    //size 表示每个坐标有几个数字,如3(x,y,z)
    //Data Type- GL_FLOAT 用的是float值,也可以用整数
    //Stride 告诉OpenCL在每个坐标间可以忽略哪几个点,写0就行
    //Pointer to the Data 数据本身
